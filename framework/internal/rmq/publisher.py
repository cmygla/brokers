...
class RmqPublisher(Singleton):
    """
    RabbitMQ Publisher для отправки сообщений.
    """
    ...
    def publish(
        ...
        if properties is None:
            properties = pika.BasicProperties(
                content_type="application/json", correlation_id=str(uuid.uuid4()), )

        message_body = json.dumps(message).encode("utf-8")
        self._channel.basic_publish(
            exchange=exchange, routing_key=routing_key, body=message_body, properties=properties, )
